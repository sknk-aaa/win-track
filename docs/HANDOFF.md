...
- ロック画面Widgetの勝率が改行されないよう、左カラムを優先しボタン幅を固定する調整を追加。
- 設定画面では、勝敗表記切替をアプリアイコン直前へ移動し、ウィジェット枠説明にホーム画面Widgetは3種類まで置けることを追記。
- アプリ名は「勝率カウンター」。
- 海外向け展開は後回し。
...
1. 次のTestFlightで、ロック画面Widgetの `50.0%` などの勝率が一行で表示されるか確認する。
2. 設定画面で勝敗表記切替の位置が目立ちすぎず、アプリアイコン直前に自然に見えるか確認する。
3. ウィジェット枠説明が、ロック画面の枠1とホーム画面の3種類設置を理解しやすいか確認する。
4. Widgetから勝ち/負けを押した後、アプリのカウンター数値と履歴へ反映されるか継続確認する。
